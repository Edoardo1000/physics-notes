...
>$$
>The second equality follows because the sum of the generators projects onto the first four components. We now expand the sine, writing $H = \langle H \rangle + h$, we have at leading order (the next order is used for interactions)
>$$
...
>$$
>	A_{\mu} \to g_{5} A_{\mu} g_{5}^{-1},\quad g_{5}\in SO(5),
>$$
...
$$
	V_{\mathrm{gauge}}(h) \sim \frac{3}{2} \int \frac{d^{4}q}{(2\pi )^{4}}  \log K_{A}(q,H),
$$
...
$$
	V_{\mathrm{fer mions}}(h) \sim -2N_{c} \int \frac{d^{4}q}{(2\pi)^{4}} \det K_{f}(q,H).
$$
Because of the $\sin ^{2} \frac{H}{f}$ dependence, a purely bosonic contribution gives minima of the potential at $\pi k f \geq f$, which is a problem. Thus, we need the minus sign coming from fermion loops. 
...
The Coleman-Weinberg potential is suppressed by the scale of strong resonances $m_{*}$, since 
$$
